...
-- before others. Any participants who could not be matched are returned with the "partner" column set to null.
--
-- Example Usage:
...
BEGIN
    ...
    FOR v_user IN (
        ...
        FROM members
        WHERE channel_id = p_channel_id AND is_active AND has_gender_preference
        ORDER BY RANDOM()
    ) LOOP
